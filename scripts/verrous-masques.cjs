/* CE QUI EST VERROUILLÉ NE S'AFFICHE PAS — la mesure, pas l'œil.
 *
 * ⚠️ POURQUOI CE SCRIPT EXISTE. La règle est arbitrée depuis le 2026-08-27
 * (« on devrait masquer tout ce qui est verrouillé »), et elle avait été
 * mesurée : « plus aucun 🔒 dans le DOM d'un joueur NEUF ». Le trou est dans
 * l'énoncé — un joueur neuf n'a pas l'Atelier, donc il ne voit jamais sa barre
 * de menus. Mesuré depuis en JOUANT : un cadenas y attendait de l'acte 2 à
 * l'acte 7, c'est-à-dire presque toute la partie. Une fixture ne joue pas le
 * jeu ; celui-ci le joue, comme `parcours-carriere.cjs` dont il reprend le
 * pilotage.
 *
 * Il joue la carrière entière depuis un joueur neuf, garde un instantané de
 * `localStorage` à CHAQUE frontière d'acte, puis rouvre l'application sur
 * chacun et lit le DOM : accueil, Atelier, ses cinq menus, le conseil 💡, le
 * Mode jeu, la salle de répétition. Toute mention d'un module FERMÉ est
 * signalée.
 *
 * Usage : `npm run dev` dans un terminal, puis
 *   node scripts/verrous-masques.cjs
 *
 * Ce qu'il faut lire : uniquement des « rien ». Une ligne ⚠️ nomme l'écran, le
 * module fermé et le mot qui l'a trahi.
 */
...
(async () => {
  ...
  /* ---------------------------------------------------------------------
     DEUXIÈME TEMPS — on rouvre l'application sur chaque instantané et on lit
  ...
  const MOTS = {
    ...
    production: ['Production', 'Banque de séquences', 'onglet Production'],
    live: ['Mode Live'],
  };
  let fuites = 0;
  ...
  for (const snap of journal.snaps) {
    ...
    await page.evaluate((ls) => {
      localStorage.clear();
      for (const [k, v] of Object.entries(JSON.parse(ls))) localStorage.setItem(k, v);
    }, snap.ls);
    await page.reload({ waitUntil: 'networkidle' });
    await page.waitForTimeout(300);

    const ouverts = snap.modules.split(',');
    ...
    const atelier = page.locator('button', { hasText: 'Atelier' }).first();
    if (await atelier.count()) {
      await atelier.click();
    ...
    if (await jouer.count()) {
      ...
      dire('Mode jeu', await page.evaluate(() => document.body.innerText));
      const salle = page.locator('button', { hasText: 'Salle de répétition' }).first();
      if (await salle.count()) {
        await salle.click();
        await page.waitForTimeout(400);
        dire('salle de répétition', await page.evaluate(() => document.body.innerText));
      }
    }
  }

  await browser.close();
  console.log(`\n${fuites === 0 ? '✅ aucune fuite' : `⚠️ ${fuites} fuite(s)`} — erreurs console : ${errs.length ? errs.join(' / ') : 'aucune'}`);
})();
